package RDF::LinkedData::NLQuestion;
...
use RDF::LinkedData::NLQuestion::Question;
...
#			    push @semanticUnits, $line;
		...
		$question = RDF::LinkedData::NLQuestion::Question->new("docId" => $docId,
...
RDF::LinkedData::NLQuestion - Perl extension for converting Natural Language Questions in SPARQL queries
...
=item C<files> is a hashtablecontaining the name of the three files which are useful for running the converter (the configuration filename in the key C<config> and the file name where the semantic correspondance and the rewriting rules are defined in the key C<semtypecorresp>).

=item C<config> contains the configuration structure.

=item C<questions> contains the list of natural language questions.

=item C<semtypecorresp> contains the semantic correspondance and the rewriting rules to egenerate the SPARQL queries.

=item C<format> contains the format of the output. Accepted values are C<SPARQL> (the SPARQL query), C<XML> (the SPARQL query in the QALD challenge XML format), C<SPARQLANSWERS> (the answers return by the SPARQL query), C<XMLANSWERS> (the answers return by the SPARQL query in the QALD challenge XML format).

=item C<verbose> specifies the verbose level.
...
the questions and the values are objects C<RDF::LinkedData::NLQuestion::Question>).

=head2 getQuestionList()

    getQuestionList();

The method returns the list of questions (each question is an object C<RDF::LinkedData::NLQuestion::Question>).
